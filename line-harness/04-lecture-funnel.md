...
LINE2 友だち追加
  ...
  └─ [D2 20:00]  本日締切                                              #5（同上）
        │
        ▼ 動画ページ（lecture-page/）を開く → タグ「講義_視聴ページを開いた」
        ▼ 動画の下のボタン → アンケート（L Harness フォーム・LIFF）
        ▼ 送信 → タグ「講義_相談会申込」＋お礼1通（フォームの自動返信）
...
| **B. 自前ページ＋L Harness フォーム** | Cloudflare Pages に置く1枚ページ（動画埋め込み＋申込ボタン）。ボタン先は L Harness のアンケート（LIFF）。送信でタグ→シナリオ | **採用**。いつでも見られる、YouTube 限定公開でよい、文面はすべて自分で持てる |
